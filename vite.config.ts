...
import { themePreprocessorPlugin, themePreprocessorHmrPlugin } from "@zougt/vite-plugin-theme-preprocessor";//预设主题模式插件
...
export default (serve) => {
	...
	return defineConfig({
		...
		plugins: [
			vue(),
			AutoImport({
			    imports: ['vue', 'pinia', 'vue-i18n', 'vue-router', '@vueuse/core'],//自动导入
			...
			themePreprocessorPlugin({//预设主题配置
				scss: {
					// 是否启用任意主题色模式，这里不启用
					arbitraryMode: false,
					// 提供多组变量文件
					multipleScopeVars: [
					  {
						...
						path: path.resolve("src/style/theme/default/default.scss"),
					  },                               
					],
